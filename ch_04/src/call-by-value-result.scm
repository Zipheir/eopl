;;;; Call-by-value-result variant of IMPLICIT-REFS (Ex. 4.37)
...
;;
;; BUG: This doesn't correctly handle nested procedures.
(define (apply-procedure proc1 ref)
  (pmatch proc1
    ((proc ,var ,body ,env)
     (let* ((local (newref (deref ref)))
            (ext-env (extend-env var local env))
            (res (value-of body ext-env)))
       (setref! ref (deref local))
       res))))
...
(define (value-of-operand exp env)
  (pmatch exp
    ...
    (? (error 'value-of "invalid operand" exp))))
