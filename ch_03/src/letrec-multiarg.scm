...
(define-record-type letrec-exp
  (fields p-name b-vars p-body letrec-body))
...
(define-record-type (extended-env-rec extend-env-rec extended-env-rec?)
  (fields p-name b-vars p-body env))
  ...
  (cond ((empty-env? env) (report-no-binding-found search-var))
        ...
        ((extended-env-rec? env)
         (if (eqv? search-var (extended-env-rec-p-name env))
             (make-proc-val (procedure (extended-env-rec-b-vars env)
...
(define (parse sexp)
  (pmatch sexp
    ...
    ((letrec ,name ,vs = ,p-body in ,body)
     (guard (symbol? name) (pair-or-null? vs) (for-all symbol? vs))
     (make-letrec-exp name vs (parse p-body) (parse body)))
